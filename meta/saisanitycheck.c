        ...
        _In_ const sai_attr_metadata_t* md)
{
    ...
    for (; index < md->validonlylength; ++index)
    {
        ...
        {
            if (md->objecttype == SAI_OBJECT_TYPE_MIRROR_SESSION &&
                     (md->attrid == SAI_MIRROR_SESSION_ATTR_ERSPAN_SESSION_ID || md->attrid == SAI_MIRROR_SESSION_ATTR_GRE_KEY))
            {
                /*
                 * ERSPAN_SESSION_ID is valid for ERSPAN and when encap_type is ERSPAN_II or ERSPAN_III
                 * GRE_KEY is valid for ERSPAN and when encap_type is GRE_EXTENDED
                 */
            }
            else
            {
